...
@Injectable()
export class Discord2Service {
  private readonly bot: Client;
  private readonly logger = new Logger(Discord2Service.name);
  ...
  private initializeBot() {
    ...
    this.bot.login(process.env.DISCORD_TOKEN_2);
  }

  private async getActiveBot() {
    const _activeBot = await this.botRepository.findOne({
      where: { isActive: true, platform: Platform.TELEGRAM },
    });
